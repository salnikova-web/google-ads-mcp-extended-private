...
**Steps:**
1.  **Discover Fields**: Use `metadata_get_resource_metadata` with resource `campaign` or `ad_group` to ensure you have the correct field names.
2.  **Query Performance**: Use `search_search` to retrieve performance data.
...
**Steps:**
1.  **Query Impression Share**: Use `search_search` to retrieve impression share metrics.
    *   **Resource**: `campaign`
