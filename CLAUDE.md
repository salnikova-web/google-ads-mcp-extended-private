...
- Пуш ТІЛЬКИ `main`, ніколи `--all`/`--mirror` — історичні локальні гілки
  ...
  призначено для пушу.
- Реліз: branch → PR → merge на -private → bump version у pyproject →
  локально `nox -s deploy` → у репо шерингу `scripts/publish/release.sh
  vX.Y.Z` (див. «Політика публікації»); теги `vX.Y.Z` у репо шерингу
  створює release.sh, GitHub release там робить Дарина після відкриття.
...
- «Почистимо історію і відкриємо репо» (аудит 28.08.2026): PR-коміти на
  GitHub ПЕРЕЖИВАЮТЬ force-push — лишаються доступними по SHA і на
  сторінках PR, видаляє тільки GitHub Support → репо з PR-історією
  неможливо відчистити переписуванням; публічність = тільки свіжа
  orphan-історія в окремому репо → тест: у клоні репо шерингу
  `git rev-list --all --count` == кількості релізів (зараз 1). Ціна
  помилки: ім'я + повний P&L компанії назавжди в паблік-кеші GitHub.
- Перший прогін release.sh: гейт зловив 65 `.pyc` у клоні — тестовий
  прогін УСЕРЕДИНІ експортної директорії генерує `__pycache__` з
  абсолютним локальним шляхом у `co_filename` → правило:
  `PYTHONDONTWRITEBYTECODE=1` + зачистка + гейт по клону ДО коміту (все
  вже в release.sh) → тест: гейт валить будь-який не-UTF-8 файл, тому
  байткод не проходить мовчки.
